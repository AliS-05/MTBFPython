...
def main():
    ...
    for (j, k) in plot_cells:
        ...
        # so the green line spans the full axis like the gamma; the live markers
        ...
        if EXTEND_WARMUP_LEFT and len(mdates) and mdates.min() < g.cutoff.iloc[0]:
            ...
            plt.plot([x0, g0.cutoff], [g0.mtbf_hi95, g0.mtbf_hi95], color=MCMC_COLOR, lw=1.2, linestyle=":")
            plt.plot([x0, g0.cutoff], [g0.mtbf_lo95, g0.mtbf_lo95], color=MCMC_COLOR, lw=1.2, linestyle=":")
